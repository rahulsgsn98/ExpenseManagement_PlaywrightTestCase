...
const TARGET_STATUS = 'Approved-Amended' as const;

test.use({
  storageState: 'playwright/.auth/user.json'
});

test.describe('E2E-BVP-013 Branch Vendor Payment submit then Admin sets Approved-Amended', () => {
  test.beforeEach(async ({ page }) => {
    await Pagegoto(page);
    await ensureBranchManagerRoleForEnd2E(page);
  });

  test('Branch Manager submits, Super Admin sets Approved-Amended, Branch History shows Approved-Amended', async ({
    page
  }) => {
    ...
    const amount = RandomDataFaker.amount(2400, 2499);
    ...
    await test.step(`Super Admin — set status to ${TARGET_STATUS}`, async () => {
      ...
      await expect(branchVendorHistoryPage.detailsStatusDropdown).toHaveValue(TARGET_STATUS);
      await branchVendorHistoryPage.fillDetailsApprovedAmount(amount);
