...
const TARGET_STATUS = 'Pending' as const;

test.use({
  storageState: 'playwright/.auth/user.json'
});

test.describe('E2E-BVP-015 Branch Vendor Payment submit then Admin sets Pending', () => {
  test.beforeEach(async ({ page }) => {
    await Pagegoto(page);
    await ensureBranchManagerRoleForEnd2E(page);
  });

  test('Branch Manager submits, Super Admin sets Pending, Branch History shows Pending', async ({ page }) => {
    ...
    const amount = RandomDataFaker.amount(2600, 2699);
