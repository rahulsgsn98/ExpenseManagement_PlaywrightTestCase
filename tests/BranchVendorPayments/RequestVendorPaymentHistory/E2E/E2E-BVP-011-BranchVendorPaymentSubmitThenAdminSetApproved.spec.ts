...
const TARGET_STATUS = 'Approved' as const;

test.use({
  storageState: 'playwright/.auth/user.json'
});

test.describe('E2E-BVP-011 Branch Vendor Payment submit then Admin sets Approved', () => {
  test.beforeEach(async ({ page }) => {
    await Pagegoto(page);
    await ensureBranchManagerRoleForEnd2E(page);
  });

  test('Branch Manager submits, Super Admin sets Approved, Branch History shows Approved', async ({ page }) => {
    ...
    const amount = RandomDataFaker.amount(2200, 2299);
