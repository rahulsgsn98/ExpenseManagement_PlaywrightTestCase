...
test.describe('E2E-BR-010 Branch submit then Admin sets Approved-Amended', () => {
  test.beforeEach(async ({ page }) => {
    await Pagegoto(page);
    await ensureBranchManagerRoleForEnd2E(page);
  });

  test('Branch Manager submits, Super Admin sets Approved-Amended, Branch History shows Approved-Amended', async ({ page }) => {
    ...
    await test.step(`Super Admin — set status to ${TARGET_STATUS}`, async () => {
      ...
      await expect(branchHistoryPage.detailsStatusDropdown).toHaveValue(TARGET_STATUS);
      await branchHistoryPage.fillDetailsApprovedAmount(amount);
