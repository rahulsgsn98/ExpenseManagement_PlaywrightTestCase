...
/**
 * Branch Reimbursement E2E must run under the Branch Manager role.
...
export async function ensureBranchManagerRoleForEnd2E(page: Page): Promise<void> {
  ...
  if (!labels.includes(BRANCH_MANAGER_ROLE)) {
    test.skip(
      true,
      `Branch Reimbursement E2E skipped: "${BRANCH_MANAGER_ROLE}" role not available for this user (options: ${labels.join(', ')}).`
    );
    return;
  }
